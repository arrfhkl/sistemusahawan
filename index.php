...
.berita-slider-container.dragging {
    cursor: grabbing;
}
...
<script>
(function () {
    const container = document.querySelector('.berita-slider-container');
    ...
    function getCardWidth() {
        const card = wrapper.children[cardCount]; 
        const gap  = 25;
        return card.offsetWidth + gap;
    }

    // Start at the first "real" set (after the prepended clones)
    let currentX = 0;

    function jumpToReal() {
        const cw     = getCardWidth();
        const total  = cw * cardCount;
        if (currentX <= -total * 2) currentX += total;
        if (currentX >= 0)          currentX -= total;
    }
    ...
    // Initialise position silently
    currentX = -(getCardWidth() * cardCount);
    setX(currentX, false);
...
window.addEventListener('mousemove', e => {
    ...
    // Only count as a real drag if moved past threshold
...
// Block link clicks if user actually dragged
wrapper.addEventListener('click', e => {
    if (didDrag) {
        e.preventDefault();
         e.stopPropagation();
        didDrag = false; // reset after blocking
    }
}, true);
    ...
    container.addEventListener('touchmove', e => {
        ...
        touchVelocity = e.touches[0].clientX - lastTouchX;
        lastTouchX    = e.touches[0].clientX;
        currentX      = touchScrollX + dx;
        jumpToReal();
        wrapper.style.transform = `translateX(${currentX}px)`;
    }, { passive: true });

    container.addEventListener('touchend', () => {
        currentX += touchVelocity * 4;
        jumpToReal();
        setX(currentX, true);
