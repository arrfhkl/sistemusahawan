...
// Ambil kategori
$kategori = $conn->query("SELECT * FROM kategori_servis ORDER BY nama ASC");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $getKategori->execute();
    $getKategoriResult = $getKategori->get_result();
    $kategoriData = $getKategoriResult->fetch_assoc();
...
if ($stmt->execute()) {
    echo "<script>
        alert('✅ Servis berjaya ditambah!');
        window.location='profil_usahawan.php?id=$usahawan_id';
    </script>";
} else {
    echo "Ralat SQL: " . $stmt->error;
}
...
<head>
    ...
    <title>Tambah Servis- Sistem Usahawan Pahang</title>
...
<div class="container">
    <h2>Tambah Servis untuk Usahawan: <?= $nama_usahawan ?></h2>
