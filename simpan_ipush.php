...
if (move_uploaded_file($_FILES["dokumen"]["tmp_name"], $targetFile)) {
    // === Simpan ke Database ===
    $sql = "INSERT INTO permohonan_ipush (nama, ic, telefon, kategori, alamat, jumlah, tujuan, dokumen) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Permohonan berjaya dihantar!'); window.location='permohonan-ipush.php';</script>";
    } else {
