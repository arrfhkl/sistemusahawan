...
if (move_uploaded_file($_FILES["dokumen"]["tmp_name"], $targetFile)) {
    // === Simpan ke Database ===
    $sql = "INSERT INTO permohonan_ipush (nama, ic, telefon, kategori, alamat, jumlah, tujuan, dokumen, status) 
    ...
    if ($stmt->execute()) {
        echo "<script>
                alert('Permohonan iPush berjaya dihantar! Sila tunggu maklumbalas daripada pihak kami.');
                window.location='permohonan-ipush.php';
              </script>";
    } else {
